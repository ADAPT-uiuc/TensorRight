...
rule03 _ = do
  ...
  [rc1Size, rc1Start, rc1Length] <- newMaps ["rc1Size", "rc1Start", "rc1Length"] rclass1
  tA <- newTensor @a "A" [rclass0 --> rc0Size]
  lhs <- dynamicSlice (broadcast tA [rclass1 --> rc1Size]) $
    DySlice {
        start = [rclass0 --> rc0Start, rclass1 --> rc1Start],
        sizes = [rclass0 --> rc0Length, rclass1 --> rc1Length]
      }
  rhs <- broadcast (dynamicSlice tA $
    DySlice {
...
rule05 _ = do
  rclass <- newRClass "rclass"
  [rcSize, rcStartInner, rcStartOuter, rcLengthInner, rcLengthOuter] <- newMaps ["rcSize", "rcStartInner", "rcStartOuter", "rcLengthInner", "rcLengthOuter"] rclass

  tA <- newTensor @a "A" [rclass --> rcSize]
  dySliceInner <- dynamicSlice tA $
    DySlice {
      start = [rclass --> rcStartInner],
      sizes = [rclass --> rcLengthInner]
    }
  lhs <- dynamicSlice dySliceInner $
    DySlice {
