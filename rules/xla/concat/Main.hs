...
rule02 _ = do
  ...
  rhs <- concatTensor (concatTensor tA tB (ByLabel "label1")) tC (ByLabel "label1")
...
rule03 _ = do
  ...
  rhs <- concatTensor tA (concatTensor tB tC (ByLabel "label1")) (ByLabel "label1")
  rewrite "ConcatList(A, B, C) ⇒ Concat(A, Concat(B, C))" lhs rhs

rule04 :: forall a. AnyDTypeRule a
rule04 _ = do
  [rclass0, rclass1] <- newRClasses ["rclass0", "rclass1"]
  [rc0Size, rc0Start, rc0End, rc0Stride] <- newMaps ["rc0Start", "rc0Start", "rc0End", "rc0Stride"] rclass0
  [rc1Size, rc1Start1, rc1End1, rc1Start2, rc1End2, rc1Stride] <- newMaps ["rc1Size", "rc1Start1", "rc1End1", "rc1Start2", "rc1End2", "rc1Stride"] rclass1
